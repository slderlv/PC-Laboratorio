...
df1 = pd.read_csv("./results/UNI_CORR_500_01_modified.txt", sep="\t")
df2 = pd.read_csv("./results/UNI_CORR_500_02_modified.txt", sep="\t")
st.write(f"""
# {fileName1}       
         """)
st.table(df1.head(5))
...
def visualize_data_frames(data1, data2):
    ...
    freq_matrix1, x_edges1, y_edges1 = np.histogram2d(
        data1["X"], data1["Y"], bins=div)
    freq_matrix1 = freq_matrix1.transpose()
    im1 = axes[0].imshow(freq_matrix1, origin="lower", cmap="plasma", extent=[
                         x_edges1[0], x_edges1[-1], y_edges1[0], y_edges1[-1]], interpolation="nearest", aspect="auto")
    ...
    freq_matrix2, x_edges2, y_edges2 = np.histogram2d(
        data2["X"], data2["Y"], bins=div)
    freq_matrix2 = freq_matrix2.transpose()
    im2 = axes[1].imshow(freq_matrix2, origin="lower", cmap="plasma", extent=[
                         x_edges2[0], x_edges2[-1], y_edges2[0], y_edges2[-1]], interpolation="nearest", aspect="auto")
...
def visualize_histogram(data):
    ...
    freq_matrix, x_edges, y_edges = np.histogram2d(
        data["X"], data["Y"], bins=div)
    freq_matrix = freq_matrix.transpose()
    im = ax.imshow(freq_matrix, origin="lower", cmap="plasma", extent=[
                   x_edges[0], x_edges[-1], y_edges[0], y_edges[-1]], interpolation="nearest", aspect="auto")
...
def optimize_sk(df, fileName):
    ...
    plt.plot(x_data, df["Velocity_Fitted"],
